 ...
 contract VersionedInitializable {
   ...
   */
    ...
    function isConstructor() private view returns (bool) {
        ...
        assembly {
            //cs := iszero(iszero(extcodesize(addr)))
...
@notice provides the interface to fetch the Vault address
 */

contract IAddressesProvider {

    function getLendingProtocol() public view returns (address);
    function setProtocolImplementation(address _protocolContract) public;

    function getVault() public view returns (address payable);
    function setVaultImpl(address _vault) public;

    function getProtocolConfigurator() public view returns (address);
    function setProtocolConfiguratorImpl(address _configurator) public;

    function getDataProvider() public view returns (address);
    function setDataProviderImpl(address _provider) public;

    function getParametersProvider() public view returns (address);
    function setParametersProviderImpl(address _parametersProvider) public;
    ...
    function getLiquidationManager() public view returns (address);
    function setLiquidationManager(address _manager) public;

    function getLendingProtocolManager() public view returns (address);
    function setLendingProtocolManager(address _lendingProtocolManager) public;
...
contract AddressesProvider is Ownable, IAddressesProvider, AddressStorage {
    //events
    event LendingProtocolUpdated(address indexed newAddress);
    event VaultUpdated(address indexed newAddress);
    event ParametersProviderUpdated(address indexed newAddress);
    event ManagerUpdated(address indexed newAddress);
    event ProtocolConfiguratorUpdated(address indexed newAddress);
    ...
    bytes32 private constant LENDING_PROTOCOL = "LENDING_PROTOCOL";
    ...
    bytes32 private constant LENDING_PROTOCOL_MANAGER = "LENDING_PROTOCOL_MANAGER";
    bytes32 private constant LIQUIDATION_MANAGER = "LIQUIDATION_MANAGER";
    bytes32 private constant FLASHLOAN_PROVIDER = "FLASHLOAN_PROVIDER";
    ...
    /**
    * @dev returns the address of the LendingProtocol proxy
    * @return the lending protocol proxy address
    **/
    function getLendingProtocol() public view returns (address) {
        return getAddress(LENDING_PROTOCOL);
    }


    /**
    * @dev updates the implementation of the lending protocol
    * @param _protocolContract the new lending protocol implementation
    **/
    function setProtocolImplementation(address _protocolContract) public onlyOwner {
        updateImplInternal(LENDING_PROTOCOL, _protocolContract);
        emit LendingProtocolUpdated(_protocolContract);
    }
    ...
    * @return the vault proxy address
     */
    function getVault() public view returns (address payable) {
        address payable vault = address(uint160(getAddress(VAULT)));
        return vault;
    }

    /**
    * @dev updates the implementation of the vault
    * @param _vault the new vault implementation
    **/
    function setVaultImpl(address _vault) public onlyOwner {
        updateImplInternal(VAULT, _vault);
        emit VaultUpdated(_vault);
    }

    /**
    * @dev returns the address of the ProtocolConfigurator proxy
    * @return the protocol configurator proxy address
    **/
    function getProtocolConfigurator() public view returns (address) {
        return getAddress(PROTOCOL_CONFIGURATOR);
    }

    /**
    * @dev updates the implementation of the protocol configurator
    * @param _configurator the new protocol configurator implementation
    **/
    function setProtocolConfiguratorImpl(address _configurator) public onlyOwner {
        updateImplInternal(PROTOCOL_CONFIGURATOR, _configurator);
        emit ProtocolConfiguratorUpdated(_configurator);
    }

    /**
    * @dev returns the address of the DataProvider proxy
    * @return the data provider proxy address
     */
    function getDataProvider() public view returns (address) {
    ...
    /**
    * @dev updates the implementation of the data provider
    * @param _provider the new data provider implementation
    **/
    function setDataProviderImpl(address _provider) public onlyOwner {
        updateImplInternal(DATA_PROVIDER, _provider);
        emit DataProviderUpdated(_provider);
    }

    /**
    * @dev returns the address of the ParametersProvider proxy
    * @return the address of the parameters provider proxy
    **/
    function getParametersProvider() public view returns (address) {
    ...
    /**
    * @dev updates the implementation of the parameters provider
    * @param _parametersProvider the new parameters provider implementation
    **/
    function setParametersProviderImpl(address _parametersProvider) public onlyOwner {
        updateImplInternal(PARAMETERS_PROVIDER, _parametersProvider);
        emit ParametersProviderUpdated(_parametersProvider);
    }
    ...
    * @param _feeProvider the new fee provider implementation
    **/
    ...
    /**
    * @dev returns the address of the LiquidationManager. Since the manager is used
    * through delegateCall within the LendingProtocol contract, the proxy contract pattern does not work properly hence
    ...
    **/
    ...
    /**
    * @dev updates the address of the liquidation manager
    * @param _manager the new liquidation manager address
    **/
    function setLiquidationManager(address _manager) public onlyOwner {
    ...
    **/
    ...
    function getLendingProtocolManager() public view returns (address) {
        return getAddress(LENDING_PROTOCOL_MANAGER);
    }

    function setLendingProtocolManager(address _lendingProtocolManager) public onlyOwner {
        _setAddress(LENDING_PROTOCOL_MANAGER, _lendingProtocolManager);
        emit ManagerUpdated(_lendingProtocolManager);
    }
...
* @notice stores the configuration parameters of the Lending Protocol contract
**/

contract ParametersProvider is VersionedInitializable {
    ...
    /**
    * @dev initializes the ParametersProvider after it's added to the proxy
...
library VaultLibrary {
    ...
    struct ReserveData {
        ...
        **/
        ...
        uint256 decimals;
        /**
        * @dev address of the derevativeToken representing the asset
        **/
        address derevativeTokenAddress;
        /**
    ...
    **/
    function getNormalizedIncome(VaultLibrary.ReserveData storage _reserve)
        internal
        view
        returns (uint256)
    {
    ...
    * @param _self the reserve object
    * @param _derevativeTokenAddress the address of the overlying atoken contract
    * @param _decimals the number of decimals of the underlying asset
    * @param _interestRateStrategyAddress the address of the interest rate strategy contract
    **/
    function init(
        ReserveData storage _self,
        address _derevativeTokenAddress,
        uint256 _decimals,
        address _interestRateStrategyAddress
    ) external {
        require(_self.derevativeTokenAddress == address(0), "Reserve has already been initialized");
        ...
        _self.derevativeTokenAddress = _derevativeTokenAddress;
    ...
    **/
    function getCompoundedBorrowBalance(
        VaultLibrary.UserReserveData storage _self,
        VaultLibrary.ReserveData storage _reserve
    ...
    **/
    function getTotalBorrows(VaultLibrary.ReserveData storage _reserve)
        internal
        view
        returns (uint256)
    {
...
* @notice Interface for the fee provider.
**/
...
* @notice Implements functions to fetch data from the vault, and aggregate them in order to allow computation
* on the compounded balances and the account balances in ETH
**/
contract DataProvider is VersionedInitializable {
    using SafeMath for uint256;
    using WadRayMath for uint256;
    ...
    **/
    ...
    function initialize(AddressesProvider _addressesProvider) public initializer {
        ...
        vault = Vault(_addressesProvider.getVault());
    }
    ...
    * @return  totalLiquidityBalanceETH , totalCollateralBalanceETH, totalBorrowBalanceETH, 
    * totalFeesETH, currentLtv, currentLiquidationThreshold,  total borrow balances of the user in ETH.
    ...
    **/
        ...
        )
    {
        ...
        address[] memory reserves = vault.getReserves();
        ...
        for (uint256 i = 0; i < reserves.length; i++) {
            ...
            ) = vault.getUserBasicReserveData(vars.currentReserve, _user);
            ...
            ) = vault.getReserveConfiguration(vars.currentReserve);
    ...
    **/
        ...
        returns (bool)
    {
        ...
        ) = vault.getReserveConfiguration(_reserve);

        if (
            !vars.reserveUsageAsCollateralEnabled ||
            !vault.isUserUseReserveAsCollateralEnabled(_reserve, _user)
        ) {
   ...
   **/
    function calculateCollateralNeededInETH(
        ...
        uint256 reserveDecimals = vault.getReserveDecimals(_reserve);
    ...
    **/
    ...
    /**
    * @dev accessory functions to fetch data from the vault
    **/
    ...
    {
        (, ltv, liquidationThreshold, usageAsCollateralEnabled) = vault.getReserveConfiguration(
            _reserve
        );
        stableBorrowRateEnabled = vault.getReserveIsStableBorrowRateEnabled(_reserve);
        borrowingEnabled = vault.isReserveBorrowingEnabled(_reserve);
        isActive = vault.getReserveIsActive(_reserve);
        liquidationBonus = vault.getReserveLiquidationBonus(_reserve);

        rateStrategyAddress = vault.getReserveInterestRateStrategyAddress(_reserve);
    }
    ...
    {
        totalLiquidity = vault.getReserveTotalLiquidity(_reserve);
        availableLiquidity = vault.getReserveAvailableLiquidity(_reserve);
        totalBorrowsStable = vault.getReserveTotalBorrowsStable(_reserve);
        totalBorrowsVariable = vault.getReserveTotalBorrowsVariable(_reserve);
        liquidityRate = vault.getReserveCurrentLiquidityRate(_reserve);
        variableBorrowRate = vault.getReserveCurrentVariableBorrowRate(_reserve);
        stableBorrowRate = vault.getReserveCurrentStableBorrowRate(_reserve);
        averageStableBorrowRate = vault.getReserveCurrentAverageStableBorrowRate(_reserve);
        utilizationRate = vault.getReserveUtilizationRate(_reserve);
        liquidityIndex = vault.getReserveLiquidityCumulativeIndex(_reserve);
        variableBorrowIndex = vault.getReserveVariableBorrowsCumulativeIndex(_reserve);
        derevativeTokenAddress = vault.getReserveDerevativeTokenAddress(_reserve);
        lastUpdateTimestamp = vault.getReserveLastUpdate(_reserve);
    }
    ...
    function getUserReserveData(address _reserve, address _user)
        ...
        view
        returns (
            uint256 currentDerevativeTokenBalance,
    ...
    {
        currentDerevativeTokenBalance = DerevativeToken(vault.getReserveDerevativeTokenAddress(_reserve)).balanceOf(_user);
        VaultLibrary.InterestRateMode mode = vault.getUserCurrentBorrowRateMode(_reserve, _user);
        (principalBorrowBalance, currentBorrowBalance, ) = vault.getUserBorrowBalances(
            _reserve,
            _user
        );

        //default is 0, if mode == VaultLibrary.InterestRateMode.NONE
        if (mode == VaultLibrary.InterestRateMode.STABLE) {
            borrowRate = vault.getUserCurrentStableBorrowRate(_reserve, _user);
        } else if (mode == VaultLibrary.InterestRateMode.VARIABLE) {
            borrowRate = vault.getReserveCurrentVariableBorrowRate(_reserve);
        }

        borrowRateMode = uint256(mode);
        liquidityRate = vault.getReserveCurrentLiquidityRate(_reserve);
        originationFee = vault.getUserOriginationFee(_reserve, _user);
        variableBorrowIndex = vault.getUserVariableBorrowCumulativeIndex(_reserve, _user);
        lastUpdateTimestamp = vault.getUserLastUpdate(_reserve, _user);
        usageAsCollateralEnabled = vault.isUserUseReserveAsCollateralEnabled(_reserve, _user);
    }
}


/**
 * @title ERC20 derevativeToken
 *
 * @dev Implementation of the interest bearing token for the DLP protocol.
 */
contract DerevativeToken is ERC20, ERC20Detailed {
    using WadRayMath for uint256;
    ...
    **/
    ...
    AddressesProvider private addressesProvider;
    Vault private vault;
    LendingProtocol private protocolContract;
    DataProvider private dataProvider;

    modifier onlyLendingProtocol {
        require(
            msg.sender == address(protocolContract),
            "The caller of this function must be a lending protocol"
        );
        _;
    }
    ...
    constructor(
        ...
        addressesProvider = _addressesProvider;
        vault = Vault(addressesProvider.getVault());
        protocolContract = LendingProtocol(addressesProvider.getLendingProtocol());
        dataProvider = DataProvider(addressesProvider.getDataProvider());
    ...
    **/
    ...
    /**
    * @dev redeems derevativeToken for the underlying asset
    ...
    **/
    function redeem(uint256 _amount) external {
        ...
        protocolContract.redeemUnderlying(
    ...
    /**
     * @dev mints token in the event of users depositing the underlying asset into the lending protocol
     * only lending protocols can call this function
     * @param _account the address receiving the minted tokens
     * @param _amount the amount of tokens to mint
     */
    function mintOnDeposit(address _account, uint256 _amount) external onlyLendingProtocol {
     ...
     * Transfer of the liquidated asset is executed by the lending protocol contract.
     * only lending protocols can call this function
     * @param _account the address from which burn the derevativeTokens
     * @param _value the amount to burn
     **/
    function burnOnLiquidation(address _account, uint256 _value) external onlyLendingProtocol {
    ...
    /**
     * @dev transfers tokens in the event of a borrow being liquidated, in case the liquidators reclaims the derevativeToken
     *      only lending protocols can call this function
     * @param _from the address from which transfer the derevativeTokens
     * @param _to the destination address
     * @param _value the amount to transfer
     **/
    function transferOnLiquidation(address _from, address _to, uint256 _value) external onlyLendingProtocol {
    ...
    **/
    ...
    /**
    * @dev calculates the total supply of the specific derevativeToken
    ...
    **/
    function totalSupply() public view returns(uint256) {
        ...
        return currentSupplyPrincipal
            .wadToRay()
            .rayMul(vault.getReserveNormalizedIncome(underlyingAssetAddress))
            .rayToWad();
    }
    ...
    **/
    function cumulateBalanceInternal(address _user)
        ...
        uint256 index = userIndexes[_user] = vault.getReserveNormalizedIncome(underlyingAssetAddress);
        return (
    ...
    **/
    ...
    /**
    * @dev executes the transfer of derevativeTokens, invoked by both _transfer() and
...
* @notice Interface for the borrow rate oracle. Provides the average market borrow rate to be used as a base for the stable borrow rate calculations
**/
...
/**
* @title LendingProtocol contract
* @notice Implements the actions of the LendingProtocol, and exposes accessory methods to fetch the users and reserve data
**/

contract LendingProtocol is ReentrancyGuard, VersionedInitializable {
    using SafeMath for uint256;
    using WadRayMath for uint256;
    using Address for address;
    ...
    **/
    ...
    /**
    * @dev these events are not emitted directly by the LendingProtocol
    * but they are declared here as the LiquidationManager
    * is executed using a delegateCall().
    * This allows to have the events in the generated ABI for LendingProtocol.
    **/
    ...
    * @param _receiveDerevativeToken true if the liquidator wants to receive derevativeTokens, false otherwise
    * @param _timestamp the timestamp of the action
    **/
    event LiquidationCall(
        ...
        address _liquidator,
        bool _receiveDerevativeToken,
        uint256 _timestamp
    );

    /**
    * @dev functions affected by this modifier can only be invoked by the
    * derevativeToken.sol contract
    * @param _reserve the address of the reserve
    **/
    modifier onlyOverlyingDerevativeToken(address _reserve) {
        require(
            msg.sender == vault.getReserveDerevativeTokenAddress(_reserve),
            "The caller of this function can only be the derevativeToken contract of this reserve"
        );
        _;
    }
    ...
    **/
    ...
    uint256 public constant PROTOCOL_REVISION = 0x5;
    ...
    /**
    * @dev this function is invoked by the proxy contract when the LendingProtocol contract is added to the
    * AddressesProvider.
    ...
    **/
    function initialize(AddressesProvider _addressesProvider) public initializer {
        ...
        parametersProvider = ParametersProvider(
            addressesProvider.getParametersProvider()
        );
        feeProvider = IFeeProvider(addressesProvider.getFeeProvider());
    }

    /**
    * @dev deposits The underlying asset into the reserve. A corresponding amount of the overlying asset (derevativeTokens)
    * is minted.
    * @param _reserve the address of the reserve
    * @param _amount the amount to be deposited
    ...
    **/
    ...
    {
        DerevativeToken derevativeToken = DerevativeToken(vault.getReserveDerevativeTokenAddress(_reserve));

        bool isFirstDeposit = derevativeToken.balanceOf(msg.sender) == 0;

        vault.updateStateOnDeposit(_reserve, msg.sender, _amount, isFirstDeposit);

        //minting DerevativeToken to user 1:1 with the specific exchange rate
        derevativeToken.mintOnDeposit(msg.sender, _amount);

        //transfer to the vault contract
        vault.transferToReserve.value(msg.value)(_reserve, msg.sender, _amount);
    ...
    * This function is executed by the overlying derevativeToken contract in response to a redeem action.
    * @param _reserve the address of the reserve
    ...
    **/
    function redeemUnderlying(
        ...
        uint256 _amount,
        uint256 _derevativeTokenBalanceAfterRedeem
    )
        external
        nonReentrant
        onlyOverlyingDerevativeToken(_reserve)
        onlyActiveReserve(_reserve)
        onlyAmountGreaterThanZero(_amount)
    {
        uint256 currentAvailableLiquidity = vault.getReserveAvailableLiquidity(_reserve);
        require(
            currentAvailableLiquidity >= _amount,
            "There is not enough liquidity available to redeem"
        );

        vault.updateStateOnRedeem(_reserve, _user, _amount, _derevativeTokenBalanceAfterRedeem == 0);

        vault.transferToUser(_reserve, _user, _amount);
    ...
    */
    ...
    struct BorrowLocalVars {
        ...
        VaultLibrary.InterestRateMode rateMode;
        bool healthFactorBelowThreshold;
    }
    ...
    **/
        ...
        onlyAmountGreaterThanZero(_amount)
    {
        ...
        require(vault.isReserveBorrowingEnabled(_reserve), "Reserve is not enabled for borrowing");
        //validate interest rate mode
        require(
            uint256(VaultLibrary.InterestRateMode.VARIABLE) == _interestRateMode ||
                uint256(VaultLibrary.InterestRateMode.STABLE) == _interestRateMode,
            "Invalid interest rate mode selected"
        );

        //cast the rateMode to vaultLibrary.interestRateMode
        vars.rateMode = VaultLibrary.InterestRateMode(_interestRateMode);

        //check that the amount is available in the reserve
        vars.availableLiquidity = vault.getReserveAvailableLiquidity(_reserve);
        ...
        **/
        ...
        if (vars.rateMode == VaultLibrary.InterestRateMode.STABLE) {
            //check if the borrow mode is stable and if stable rate borrowing is enabled on this reserve
            require(
                vault.isUserAllowedToBorrowAtStable(_reserve, msg.sender, _amount),
        ...
        (vars.finalUserBorrowRate, vars.borrowBalanceIncrease) = vault.updateStateOnBorrow(
            _reserve,
            msg.sender,
            _amount,
            vars.borrowFee,
        ...
        vault.transferToUser(_reserve, msg.sender, _amount);
    ...
    **/
        ...
        onlyAmountGreaterThanZero(_amount)
    {
        ...
        (
            ...
            vars.compoundedBorrowBalance,
            vars.borrowBalanceIncrease
        ) = vault.getUserBorrowBalances(_reserve, _onBehalfOf);

        vars.originationFee = vault.getUserOriginationFee(_reserve, _onBehalfOf);
        ...
        if (vars.paybackAmount <= vars.originationFee) {
            ...
            vault.transferToFeeCollectionAddress.value(vars.isETH ? vars.paybackAmount : 0)(
                _reserve,
                msg.sender,
                vars.paybackAmount,
        ...
        if(vars.originationFee > 0) {
            vault.transferToFeeCollectionAddress.value(vars.isETH ? vars.originationFee : 0)(
                _reserve,
                msg.sender,
                vars.originationFee,
                addressesProvider.getTokenDistributor()
        ...
        vault.transferToReserve.value(vars.isETH ? msg.value.sub(vars.originationFee) : 0)(
            _reserve,
            msg.sender,
            vars.paybackAmountMinusFees
        );
    ...
    **/
    function swapBorrowRateMode(address _reserve)
        ...
        onlyUnfreezedReserve(_reserve)
    {
        (uint256 principalBorrowBalance, uint256 compoundedBorrowBalance, uint256 borrowBalanceIncrease) = vault
        ...
        VaultLibrary.InterestRateMode currentRateMode = vault.getUserCurrentBorrowRateMode(
            _reserve,
            msg.sender
        );

        if (currentRateMode == VaultLibrary.InterestRateMode.VARIABLE) {
            /**
            ...
            **/
            require(
                vault.isUserAllowedToBorrowAtStable(_reserve, msg.sender, compoundedBorrowBalance),
                "User cannot borrow the selected amount at stable"
            );
        }

        (VaultLibrary.InterestRateMode newRateMode, uint256 newBorrowRate) = vault
    ...
    **/
    function rebalanceStableBorrowRate(address _reserve, address _user)
        ...
        onlyActiveReserve(_reserve)
    {
        (uint256 principalBalance, uint256 compoundedBalance, uint256 borrowBalanceIncrease) = vault.getUserBorrowBalances(
            _reserve,
            _user
        );
        ...
        VaultLibrary.InterestRateMode rateMode = vault.getUserCurrentBorrowRateMode(_reserve, _user);
        require(
            rateMode ==
                VaultLibrary.InterestRateMode.STABLE,
            "The user borrow is variable and cannot be rebalanced"
        );

        uint256 userCurrentStableRate = vault.getUserCurrentStableBorrowRate(_reserve, _user);
        uint256 liquidityRate = vault.getReserveCurrentLiquidityRate(_reserve);
    ...
    **/
    function setUserUseReserveAsCollateral(address _reserve, bool _useAsCollateral)
        ...
        onlyUnfreezedReserve(_reserve)
    {
        uint256 underlyingBalance = vault.getUserUnderlyingAssetBalance(_reserve, msg.sender);
        ...
        vault.setUserUseReserveAsCollateral(_reserve, msg.sender, _useAsCollateral);
    ...
    * @param _receiveDerevativeToken true if the liquidators wants to receive the derevativeTokens, false if
    ...
    **/
    function liquidationCall(
        ...
        address liquidationManager = addressesProvider.getLiquidationManager();
    ...
    /**
    * @dev allows smartcontracts to access the liquidity of the protocolContract within one transaction,
    ...
    **/
        ...
        onlyAmountGreaterThanZero(_amount)
    {
        ...
        //we avoid using the getAvailableLiquidity() function in Vault to save gas
        uint256 availableLiquidityBefore = _reserve == EthAddressLib.ethAddress()
            ? address(vault).balance
            : IERC20(_reserve).balanceOf(address(vault));
        ...
        vault.transferToUser(_reserve, userPayable, _amount);

        //execute action of the receiver
        receiver.executeOperation(_reserve, _amount, amountFee, _params);

        //check that the actual balance of the vault contract includes the returned amount
    ...
    /**
    * @dev accessory functions to fetch data from the vault contract
    **/
    ...
    **/
    function requireReserveActiveInternal(address _reserve) internal view {
        require(vault.getReserveIsActive(_reserve), "Action requires an active reserve");
    }
    ...
    **/
    function requireReserveNotFreezedInternal(address _reserve) internal view {
        require(!vault.getReserveIsFreezed(_reserve), "Action requires an unfreezed reserve");
    }
...
* @notice Holds the state of the lending protocol and all the funds deposited
* @dev NOTE: The vault does not enforce security checks on the update of the state
* (eg, updateStateOnBorrow() does not enforce that borrowed is enabled on the reserve).
* The check that an action can be performed is a duty of the overlying LendingProtocol contract.
**/

contract Vault is VersionedInitializable {
    using SafeMath for uint256;
    using WadRayMath for uint256;
    using VaultLibrary for VaultLibrary.ReserveData;
    using VaultLibrary for VaultLibrary.UserReserveData;
    ...
    **/
    ...
    /**
    * @dev only lending protocols can use functions affected by this modifier
    **/
    modifier onlyLendingProtocol {
        require(lendingProtocolAddress == msg.sender, "The caller must be a lending protocol contract");
        _;
    }

    /**
    * @dev only protocols configurator can use functions affected by this modifier
    **/
    modifier onlyProtocolConfigurator {
        require(
            addressesProvider.getProtocolConfigurator() == msg.sender,
            "The caller must be a protocol configurator contract"
        );
        _;
    }

    mapping(address => VaultLibrary.ReserveData) internal reserves;
    mapping(address => mapping(address => VaultLibrary.UserReserveData)) internal usersReserveData;

    address[] public reservesList;

    uint256 public constant VAULT_REVISION = 0x7;
    ...
    **/
    ...
    /**
    * @dev initializes the Vault contract, invoked upon registration on the AddressesProvider
    ...
    **/
    ...
    /**
    * @dev updates the state of the vault as a result of a deposit action
    ...
    **/
    ...
    /**
    * @dev updates the state of the vault as a result of a redeem action
    ...
    **/
    ...
    /**
    * @dev updates the state of the vault as a result of a flashloan action
    ...
    **/
    ...
    /**
    * @dev updates the state of the vault as a consequence of a borrow action.
    ...
    **/
    function updateStateOnBorrow(
        ...
        uint256 _borrowFee,
        VaultLibrary.InterestRateMode _rateMode
    ) external onlyLendingProtocol returns (uint256, uint256) {
    ...
    /**
    * @dev updates the state of the vault as a consequence of a repay action.
    ...
    **/
    ...
    /**
    * @dev updates the state of the vault as a consequence of a swap rate action.
    * @param _reserve the address of the reserve on which the user is repaying
    * @param _user the address of the borrower
    ...
    **/
    function updateStateOnSwapRate(
        ...
        uint256 _balanceIncrease,
        VaultLibrary.InterestRateMode _currentRateMode
    ) external onlyLendingProtocol returns (VaultLibrary.InterestRateMode, uint256) {
        ...
        VaultLibrary.InterestRateMode newRateMode = updateUserStateOnSwapRateInternal(
            _reserve,
            _user,
            _balanceIncrease,
            _currentRateMode
    ...
    /**
    * @dev updates the state of the vault as a consequence of a liquidation action.
    ...
    * @param _balanceIncrease the accrued interest on the borrowed amount
    * @param _liquidatorReceivesDerevativeToken true if the liquidator will receive derevativeTokens, false otherwise
    **/
    function updateStateOnLiquidation(
        ...
        uint256 _liquidatedCollateralForFee,
        uint256 _balanceIncrease,
        bool _liquidatorReceivesDerevativeToken
    ) external onlyLendingProtocol {
        ...
        if (!_liquidatorReceivesDerevativeToken) {
    ...
    /**
    * @dev updates the state of the vault as a consequence of a stable rate rebalance
    ...
    **/
    function setUserUseReserveAsCollateral(address _reserve, address _user, bool _useAsCollateral)
        ...
        onlyLendingProtocol
    {
        VaultLibrary.UserReserveData storage user = usersReserveData[_user][_reserve];
    ...
    **/
    function () external payable {
        //only contracts can send ETH to the vault
        require(msg.sender.isContract(), "Only contracts can send ether to the Vault");
    ...
    **/
    ...
    ) external payable onlyLendingProtocol {
    ...
    * needed to calculate the global account data in the DataProvider
    * @param _reserve the address of the reserve
    * @param _user the address of the user
    ...
    **/
    ...
    {
        VaultLibrary.ReserveData storage reserve = reserves[_reserve];
        VaultLibrary.UserReserveData storage user = usersReserveData[_user][_reserve];
    ...
    **/
    ...
    /**
    * @dev gets the underlying asset balance of a user based on the corresponding derevativeToken balance.
    * @param _reserve the reserve address
    ...
    **/
    ...
    function getUserUnderlyingAssetBalance(address _reserve, address _user)
        ...
        returns (uint256)
    {
        DerevativeToken derevativeToken = DerevativeToken(reserves[_reserve].derevativeTokenAddress);
        return derevativeToken.balanceOf(_user);
    ...
    **/
    ...
    /**
    * @dev gets the derevativeToken contract address for the reserve
    * @param _reserve the reserve address
    * @return the address of the derevativeToken contract
    **/

    function getReserveDerevativeTokenAddress(address _reserve) public view returns (address) {
        VaultLibrary.ReserveData storage reserve = reserves[_reserve];
        return reserve.derevativeTokenAddress;
    }

    /**
    * @dev gets the available liquidity in the reserve. The available liquidity is the balance of the vault contract
    * @param _reserve the reserve address
    ...
    **/
    ...
    /**
    * @dev gets the total liquidity in the reserve. The total liquidity is the balance of the vault contract + total borrows
    * @param _reserve the reserve address
    ...
    * It's used in the DataProvider specifically to save gas, and avoid
    ...
    **/
    ...
    /**
    * @return the array of reserves configured on the vault
    **/
    function getReserves() external view returns (address[] memory) {
        return reservesList;
    }
    ...
    **/
    ...
    function getUserCurrentBorrowRateMode(address _reserve, address _user)
        public
        view
        returns (VaultLibrary.InterestRateMode)
    {
        VaultLibrary.UserReserveData storage user = usersReserveData[_user][_reserve];

        if (user.principalBorrowBalance == 0) {
            return VaultLibrary.InterestRateMode.NONE;
        }

        return
            user.stableBorrowRate > 0
            ? VaultLibrary.InterestRateMode.STABLE
            : VaultLibrary.InterestRateMode.VARIABLE;
    }
    ...
    **/
    function getUserCurrentBorrowRate(address _reserve, address _user)
        ...
        returns (uint256)
    {
        VaultLibrary.InterestRateMode rateMode = getUserCurrentBorrowRateMode(_reserve, _user);

        if (rateMode == VaultLibrary.InterestRateMode.NONE) {
            return 0;
        }

        return
            rateMode == VaultLibrary.InterestRateMode.STABLE
    ...
    **/
        ...
        returns (uint256, uint256, uint256)
    {
        ...
        uint256 compoundedBalance = VaultLibrary.getCompoundedBorrowBalance(
            user,
            reserves[_reserve]
        );
    ...
    **/
    ...
    /**
    * @dev updates the vault configuration
    **/
    function refreshConfiguration() external onlyProtocolConfigurator {
        refreshConfigInternal();
    }
    ...
    * @dev initializes a reserve
    * @param _reserve the address of the reserve
    * @param _derevativeTokenAddress the address of the overlying derevativeToken contract
    ...
    **/
    function initReserve(
        ...
        uint256 _decimals,
        address _interestRateStrategyAddress
    ) external onlyProtocolConfigurator {
        reserves[_reserve].init(_derevativeTokenAddress, _decimals, _interestRateStrategyAddress);
    ...
    **/
    function removeLastAddedReserve(address _reserveToRemove)
     external onlyProtocolConfigurator {
        ...
        reserves[lastReserve].derevativeTokenAddress = address(0);
    ...
    **/
    ...
    function disableBorrowingOnReserve(address _reserve) external onlyProtocolConfigurator {
    ...
    * @param _reserve the address of the reserve
    **/
    function disableReserveAsCollateral(address _reserve) external onlyProtocolConfigurator {
    ...
    * @param _reserve the address of the reserve
    **/
    function enableReserveStableBorrowRate(address _reserve) external onlyProtocolConfigurator {
        VaultLibrary.ReserveData storage reserve = reserves[_reserve];
    ...
    * @param _reserve the address of the reserve
    **/
    function disableReserveStableBorrowRate(address _reserve) external onlyProtocolConfigurator {
        VaultLibrary.ReserveData storage reserve = reserves[_reserve];
    ...
    * @param _reserve the address of the reserve
    **/
    function activateReserve(address _reserve) external onlyProtocolConfigurator {
        VaultLibrary.ReserveData storage reserve = reserves[_reserve];
    ...
    * @param _reserve the address of the reserve
    **/
    function deactivateReserve(address _reserve) external onlyProtocolConfigurator {
        VaultLibrary.ReserveData storage reserve = reserves[_reserve];
    ...
    * @param _reserve the address of the reserve
    **/
    function freezeReserve(address _reserve) external onlyProtocolConfigurator {
        VaultLibrary.ReserveData storage reserve = reserves[_reserve];
    ...
    * @param _reserve the address of the reserve
    **/
    function unfreezeReserve(address _reserve) external onlyProtocolConfigurator {
        VaultLibrary.ReserveData storage reserve = reserves[_reserve];
    ...
    **/
    ...
    function updateUserStateOnBorrowInternal(
        ...
        if (_rateMode == VaultLibrary.InterestRateMode.STABLE) {
            //stable
            //reset the user variable index, and update the stable rate
            user.stableBorrowRate = reserve.currentStableBorrowRate;
            user.lastVariableBorrowCumulativeIndex = 0;
        } else if (_rateMode == VaultLibrary.InterestRateMode.VARIABLE) {
            //variable
    ...
    **/
    ...
    function updateReserveStateOnRepayInternal(
        ...
        VaultLibrary.InterestRateMode borrowRateMode = getUserCurrentBorrowRateMode(_reserve, _user);

        //update the indexes
        reserves[_reserve].updateCumulativeIndexes();

        //compound the cumulated interest to the borrow balance and then subtracting the payback amount
        if (borrowRateMode == VaultLibrary.InterestRateMode.STABLE) {
    ...
    **/
    function updateReserveStateOnSwapRateInternal(
        ...
        if (_currentRateMode == VaultLibrary.InterestRateMode.STABLE) {
        ...
        } else if (_currentRateMode == VaultLibrary.InterestRateMode.VARIABLE) {
            //swap to stable
    ...
    **/
    ...
    function updateUserStateOnSwapRateInternal(
        ...
        uint256 _balanceIncrease,
        VaultLibrary.InterestRateMode _currentRateMode
    ) internal returns (VaultLibrary.InterestRateMode) {
        VaultLibrary.UserReserveData storage user = usersReserveData[_user][_reserve];
        VaultLibrary.ReserveData storage reserve = reserves[_reserve];

        VaultLibrary.InterestRateMode newMode = VaultLibrary.InterestRateMode.NONE;

        if (_currentRateMode == VaultLibrary.InterestRateMode.VARIABLE) {
            //switch to stable
            newMode = VaultLibrary.InterestRateMode.STABLE;
            user.stableBorrowRate = reserve.currentStableBorrowRate;
            user.lastVariableBorrowCumulativeIndex = 0;
        } else if (_currentRateMode == VaultLibrary.InterestRateMode.STABLE) {
            newMode = VaultLibrary.InterestRateMode.VARIABLE;
            user.stableBorrowRate = 0;
            user.lastVariableBorrowCumulativeIndex = reserve.lastVariableBorrowCumulativeIndex;
        } else {
    ...
    **/
    ...
    function updatePrincipalReserveStateOnLiquidationInternal(
        ...
        VaultLibrary.ReserveData storage reserve = reserves[_principalReserve];
        VaultLibrary.UserReserveData storage user = usersReserveData[_user][_principalReserve];

        //update principal reserve data
        reserve.updateCumulativeIndexes();

        VaultLibrary.InterestRateMode borrowRateMode = getUserCurrentBorrowRateMode(
            _principalReserve,
            _user
        );
    ...
    **/
    function updateUserStateOnLiquidationInternal(
        ...
        if (
            getUserCurrentBorrowRateMode(_reserve, _user) == VaultLibrary.InterestRateMode.VARIABLE
        ) {
            user.lastVariableBorrowCumulativeIndex = reserve.lastVariableBorrowCumulativeIndex;
        }
    ...
    **/
    function updateReserveTotalBorrowsByRateModeInternal(
        ...
        uint256 _balanceIncrease,
        uint256 _amountBorrowed,
        VaultLibrary.InterestRateMode _newBorrowRateMode
    ) internal {
        VaultLibrary.InterestRateMode previousRateMode = getUserCurrentBorrowRateMode(
            _reserve,
            _user
        );
        VaultLibrary.ReserveData storage reserve = reserves[_reserve];

        if (previousRateMode == VaultLibrary.InterestRateMode.STABLE) {
            VaultLibrary.UserReserveData storage user = usersReserveData[_user][_reserve];
            reserve.decreaseTotalBorrowsStableAndUpdateAverageRate(
                _principalBalance,
                user.stableBorrowRate
            );
        } else if (previousRateMode == VaultLibrary.InterestRateMode.VARIABLE) {
        ...
        if (_newBorrowRateMode == VaultLibrary.InterestRateMode.STABLE) {
            reserve.increaseTotalBorrowsStableAndUpdateAverageRate(
                newPrincipalAmount,
                reserve.currentStableBorrowRate
            );
        } else if (_newBorrowRateMode == VaultLibrary.InterestRateMode.VARIABLE) {
    ...
    **/
    ...
    /**
    * @dev updates the internal configuration of the vault
    **/
    function refreshConfigInternal() internal {
        lendingProtocolAddress = addressesProvider.getLendingProtocol();
    }
